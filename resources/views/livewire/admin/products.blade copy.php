<div>
    ...
    <div class="flex items-center justify-between mb-2">
       <flux:heading size="xl" level="1">Manage Products </flux:heading>
        <div class="flex justify-end">
            <flux:button wire:click="create"  variant="primary"  color="indigo" class="cursor-pointer" icon="plus">Add New Product</flux:button>
        </div>
    </div>
    <div class="mb-2">
          <flux:input.group> 
                ...
                @forelse($products as $product)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800"wire:key="product-{{ $product->id }}">
                      ...
                      <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                         <flux:button wire:click="edit({{ $product->id }})" variant="filled"  class="cursor-pointer" size="sm" > Edit </flux:button>
    ...
    <div class="mt-3">
        ...
        <flux:modal name="product-form" class="w-full max-w-3xl">
            <form wire:submit.prevent="{{ $productId ? 'update' : 'save' }}" enctype="multipart/form-data" class="space-y-6">
                <div>
                    <flux:heading size="lg">{{ $productId ? 'Edit Product' : 'Add New Product' }}</flux:heading>
                    <flux:subheading class="mt-2">
                        {{ $productId ? 'Update the product details below.' : 'Fill in the product details below.' }}
                    </flux:subheading>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    @elseif ($currentImageUrl)
                        <img src="{{ $currentImageUrl }}" class="h-20 w-20 object-cover rounded-lg border" />
                    @endif
                ...
                <div class="flex justify-end gap-3 pt-4 border-t dark:border-zinc-700">
                    <flux:modal.close>
                        <flux:button type="button" variant="ghost" class="cursor-pointer">Cancel</flux:button>
                    </flux:modal.close>
